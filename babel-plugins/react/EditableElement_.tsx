...
// The inject-source-location babel plugin stamps these props onto every JSX
// element for the visual editor. They must never reach the rendered component:
// on web, react-native-web forwards unknown props to the DOM and React logs
// "React does not recognize the `__sourceLocation` prop on a DOM element".
// Overriding to undefined via cloneElement drops them (cloneElement merges over
// children.props, so omitting isn't enough — they'd survive).
const STRIP_INJECTED_PROPS = {
  __sourceLocation: undefined,
  __trace: undefined,
  __dataContext: undefined,
  __contentSource: undefined,
} as const;
...
export default function EditableElement_(_props: PropsWithChildren<any>) {
  ...
  if (Platform.OS !== "web") {
    return cloneElement(children, { ...props, ...STRIP_INJECTED_PROPS });
  }
  ...
  if (type === "Image") {
    if (props.source?.uri) {
      // URL-based image: match by id + current uri
      const urlSpecificKey = `${id}:${props.source.uri}`;
      if (overwrittenProps?.[urlSpecificKey]) {
        attributes = overwrittenProps[urlSpecificKey];
      }
    } else {
      // require()-based image: source is a number, check for any id:* override
      const prefix = `${id}:`;
      const matchingKey = Object.keys(overwrittenProps ?? {}).find(k => k.startsWith(prefix));
      if (matchingKey && overwrittenProps?.[matchingKey]) {
        attributes = overwrittenProps[matchingKey];
      }
    }
  }
  ...
  if (type === "Text") {
    if (!editModeEnabled) return cloneElement(children, STRIP_INJECTED_PROPS);

    return cloneElement(children, {
      ...props,
      ...STRIP_INJECTED_PROPS,
  ...
  return cloneElement(children, STRIP_INJECTED_PROPS);
}
