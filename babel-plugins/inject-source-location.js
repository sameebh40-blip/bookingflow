...
const { EDITABLE_ELEMENTS, isIconElement, isImageElement } = require("./config.js");
  ...
  return {
    visitor: {
      JSXOpeningElement(path, state) {
        ...
        // Only inject on elements that editable-elements.js will wrap in
        // <EditableElement_> (which reads + strips these props before render).
        // Injecting on non-wrapped elements (Pressable, ScrollView, custom
        // components, …) leaks __sourceLocation/__trace/etc. straight to the
        // web DOM → "React does not recognize the `__sourceLocation` prop".
        // Keep this list in sync with wrapElementsInEdit() in
        // editable-elements.js.
        const elementName = openingName.name;
        const isWrappable =
          EDITABLE_ELEMENTS.includes(elementName) ||
          isIconElement(elementName) ||
          isImageElement(elementName);
