...
import {
  View, Text, StyleSheet, FlatList, TextInput, KeyboardAvoidingView, Platform, Image, ActivityIndicator,
} from 'react-native';
...
interface Message { id: string; text: string; is_from_venue: boolean; created_at: string; read?: boolean; }
...
/** Full chat UI. Works as a route screen OR an inline overlay — no router dependency. */
export default function ChatPanel({ venueId, onBack }: { venueId?: string; onBack: () => void }) {
  ...
  const scrollEnd = useCallback((animated = true) => { setTimeout(() => flatListRef.current?.scrollToEnd({ animated }), 80); }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase.from('messages')
        ...
        .eq('venue_id', venueId).eq('client_id', user.id).order('created_at', { ascending: true });
      if (active) { setMessages((data ?? []) as Message[]); setLoading(false); if (data && data.length) scrollEnd(false); markIncomingRead(); }
    })();
    const channel = supabase.channel(`chat:${venueId}:${user.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `venue_id=eq.${venueId}` }, (payload) => {
        ...
        setMessages((prev) => prev.some((m) => m.id === msg.id) ? prev
          : [...prev.filter((m) => !(m.id.startsWith('tmp-') && m.text === msg.text && m.is_from_venue === msg.is_from_venue)), msg as Message]);
        scrollEnd();
        if (msg.is_from_venue) markIncomingRead();
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'messages', filter: `venue_id=eq.${venueId}` }, (payload) => {
        const msg = payload.new as any;
        if (msg.client_id !== user.id) return;
  ...
  const handleSend = useCallback(async () => {
    ...
    setMessages((prev) => [...prev, temp]); scrollEnd();
    try {
      const { error } = await supabase.from('messages').insert({ venue_id: venueId, sender_id: user.id, client_id: user.id, text, is_from_venue: false });
      if (error) { setMessages((prev) => prev.filter((m) => m.id !== temp.id)); setInputText(text); }
    } catch { setMessages((prev) => prev.filter((m) => m.id !== temp.id)); setInputText(text); }
    finally { setSending(false); }
  }, [inputText, venueId, user, scrollEnd]);

  const rows: Row[] = []; let lastDay = '';
  for (const m of messages) { const k = dayKey(m.created_at); if (k !== lastDay) { rows.push({ id: `sep-${k}`, _sep: dayLabel(m.created_at) }); lastDay = k; } rows.push(m); }
  const initials = (venueName || 'V').trim().charAt(0).toUpperCase();

  const renderItem = ({ item }: { item: Row }) => {
    if ('_sep' in item) return <View style={styles.sepWrap}><View style={styles.sepPill}><Text style={styles.sepText}>{item._sep}</Text></View></View>;
      ...
      <View style={[styles.row, isUser && styles.rowUser]}>
        <View style={[styles.bubble, isUser ? styles.bubbleUser : styles.bubbleVenue]}>
          ...
          <View style={styles.metaRow}>
            ...
            {isUser && !item.id.startsWith('tmp-') && (item.read ? <CheckCheck size={14} color="#0A6B2E" /> : <Check size={14} color="rgba(10,10,15,0.5)" />)}
          </View>
        </View>
      </View>
    );
  ...
  return (
    <KeyboardAvoidingView style={[styles.container, { paddingTop: insets.top }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        <AnimatedPressable onPress={onBack} style={styles.backBtn}><ArrowLeft size={20} color={MADAR_COLORS.text} /></AnimatedPressable>
        {venueImage ? <Image source={{ uri: venueImage }} style={styles.avatar} resizeMode="cover" />
      ...
      {!user ? (
        ...
        <FlatList ref={flatListRef} data={rows} renderItem={renderItem} keyExtractor={(item) => item.id}
          contentContainerStyle={styles.list} onContentSizeChange={() => scrollEnd(false)} showsVerticalScrollIndicator={false}
          ListEmptyComponent={<View style={styles.center}><Text style={{ fontSize: 40 }}>💬</Text><Text style={styles.dim}>{'No messages yet.\nSay hello to start the conversation.'}</Text></View>} />
      )}
      <View style={[styles.inputBar, { paddingBottom: insets.bottom + 8 }]}>
        <TextInput style={styles.input} placeholder="Type a message…" placeholderTextColor={MADAR_COLORS.textTertiary}
          value={inputText} onChangeText={setInputText} multiline editable={!!user} />
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: MADAR_COLORS.border },
  backBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: MADAR_COLORS.surface, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: MADAR_COLORS.border },
  ...
  inputBar: { flexDirection: 'row', alignItems: 'flex-end', gap: 10, paddingHorizontal: 16, paddingTop: 12, backgroundColor: MADAR_COLORS.surface, borderTopWidth: 1, borderTopColor: MADAR_COLORS.border },
  input: { flex: 1, backgroundColor: MADAR_COLORS.background, borderRadius: 22, paddingHorizontal: 16, paddingVertical: 10, fontSize: 15, color: MADAR_COLORS.text, maxHeight: 110, borderWidth: 1, borderColor: MADAR_COLORS.border },
