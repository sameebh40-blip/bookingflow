// iOS-specific home screen — full implementation (cannot re-export from index.tsx due to circular import)
  ...
  const fetchVenues = async () => {
    ...
    try {
      ...
      if (error || !data || data.length === 0) {
        console.log('[Home] Using mock venues:', error?.message);
        setVenues(MOCK_VENUES);
      } else {
        console.log('[Home] Loaded', data.length, 'venues');
        setVenues(data);
      }
    } catch (err) {
      console.log('[Home] Exception, using mock:', err);
      setVenues(MOCK_VENUES);
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoriesContainer} style={styles.categoriesScroll}>
        {CATEGORIES.map((cat) => {
          ...
          return (
            <AnimatedPressable key={cat.id} onPress={() => handleCategoryPress(cat.id, cat.label)} style={[styles.categoryChip, isActive && styles.categoryChipActive]}>
              <CatIcon size={14} color={isActive ? MADAR_COLORS.background : MADAR_COLORS.textSecondary} strokeWidth={2} />
      ...
      <View style={styles.section}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.rebookScroll}>
          {MOCK_REBOOK.map((item, index) => (
            <AnimatedListItem key={item.id} index={index}>
              <AnimatedPressable onPress={() => handleRebook(item.id, item.name)} style={styles.rebookCard}>
                <Image source={resolveImageSource(item.image_url)} style={styles.rebookImage} resizeMode="cover" />
                <View style={styles.rebookInfo}>
                  <Text style={styles.rebookName} numberOfLines={1}>{item.name}</Text>
                  <Text style={styles.rebookMeta}>BHD {item.price} · {item.items} items</Text>
                  <AnimatedPressable onPress={() => handleRebook(item.id, item.name)} style={styles.rebookButton}>
        ...
        {loading ? (
          ...
          venues.map((venue, index) => (
            <AnimatedListItem key={venue.id} index={index}>
              <AnimatedPressable onPress={() => handleVenuePress(venue.id, venue.name)} style={styles.venueCard}>
                <View style={styles.venueImageContainer}>
                  <Image source={resolveImageSource(venue.image_url)} style={styles.venueImage} resizeMode="cover" />
                  <LinearGradient colors={['transparent', 'rgba(0,0,0,0.4)']} style={styles.venueImageGradient} />
                  <AnimatedPressable onPress={() => toggleFavourite(venue.id)} style={styles.heartButton}>
                    <Heart size={18} color={favourites.has(venue.id) ? MADAR_COLORS.danger : '#fff'} fill={favourites.has(venue.id) ? MADAR_COLORS.danger : 'transparent'} />
                  </AnimatedPressable>
                </View>
                <View style={styles.venueInfo}>
                  ...
                  <Text style={styles.venueMeta}>{venue.category} · {distanceText(venue.distance_km)}</Text>
...
const styles = StyleSheet.create({
  ...
  locationText: { fontSize: 14, color: MADAR_COLORS.textSecondary, fontWeight: '500' },
  greeting: { fontSize: 28, fontWeight: '700', color: MADAR_COLORS.text, letterSpacing: -0.5, marginBottom: 16 },
  searchBar: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
    backgroundColor: MADAR_COLORS.surfaceSecondary, borderRadius: 24,
    paddingHorizontal: 16, paddingVertical: 14,
    borderWidth: 1, borderColor: MADAR_COLORS.border,
  },
  searchPlaceholder: { fontSize: 15, color: MADAR_COLORS.textTertiary, flex: 1 },
  categoriesScroll: { marginHorizontal: -20, marginBottom: 24 },
  categoriesContainer: { paddingHorizontal: 20, gap: 8 },
  categoryChip: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20,
    backgroundColor: MADAR_COLORS.surface, borderWidth: 1, borderColor: MADAR_COLORS.border,
  },
  categoryChipActive: { backgroundColor: MADAR_COLORS.gold, borderColor: MADAR_COLORS.gold },
  categoryLabel: { fontSize: 13, color: MADAR_COLORS.textSecondary, fontWeight: '500' },
  ...
  rebookScroll: { gap: 12 },
  rebookCard: {
    width: 200, backgroundColor: MADAR_COLORS.surface, borderRadius: 16,
    overflow: 'hidden', borderWidth: 1, borderColor: MADAR_COLORS.border,
  },
  rebookImage: { width: '100%', height: 120 },
  rebookInfo: { padding: 12, gap: 4 },
  rebookName: { fontSize: 14, fontWeight: '700', color: MADAR_COLORS.text },
  rebookMeta: { fontSize: 12, color: MADAR_COLORS.textSecondary, marginBottom: 8 },
  rebookButton: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
    paddingVertical: 8, borderRadius: 8, borderWidth: 1, borderColor: MADAR_COLORS.goldBorder,
  },
  rebookButtonText: { fontSize: 12, color: MADAR_COLORS.gold, fontWeight: '600' },
  venueCard: {
    backgroundColor: MADAR_COLORS.surface, borderRadius: 16, overflow: 'hidden',
    marginBottom: 16, borderWidth: 1, borderColor: MADAR_COLORS.border,
  ...
  venueName: { fontSize: 17, fontWeight: '700', color: MADAR_COLORS.text, flex: 1, letterSpacing: -0.2 },
  ratingBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 3,
    backgroundColor: MADAR_COLORS.goldMuted, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8,
  },
  ratingText: { fontSize: 12, color: MADAR_COLORS.gold, fontWeight: '700' },
  venueMeta: { fontSize: 13, color: MADAR_COLORS.textSecondary },
  venueFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 },
  ...
  skeletonImage: { width: '100%', height: 180, backgroundColor: MADAR_COLORS.surfaceSecondary },
  skeletonLine: { height: 14, borderRadius: 7, backgroundColor: MADAR_COLORS.surfaceSecondary },
});
