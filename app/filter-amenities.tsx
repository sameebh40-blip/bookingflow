...
const AMENITIES = [
  'Parking available', 'Wheelchair accessible', 'WiFi', 'Air conditioning',
  'Private rooms', 'Accepts cards', 'Online booking', 'Walk-ins welcome',
  'Kids friendly', 'Late night hours',
];

export default function FilterAmenitiesScreen() {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const [selected, setSelected] = useState<Set<string>>(new Set());

  const toggle = useCallback((item: string) => {
    console.log('[FilterAmenities] Toggle:', item);
    setSelected(prev => {
  ...
  const handleApply = useCallback(() => {
    console.log('[FilterAmenities] Apply pressed, selected:', Array.from(selected));
    router.back();
  }, [selected, router]);

  const handleClear = useCallback(() => {
    console.log('[FilterAmenities] Clear pressed');
    setSelected(new Set());
  }, []);
